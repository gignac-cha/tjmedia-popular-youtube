...
export function YouTubePlayer({
  ...
  const previousQueryReference = useRef<string>(query);
  ...
  useEffect(() => {
    if (previousQueryReference.current !== query) {
      setCurrentIndex(0);
  ...
  }, [currentVideo?.videoId, loadVideo]);
  ...
  return (
    <PlayerSection ref={sectionReference}>
      ...
      <VideoContainer isExpanded={isMobileExpanded}>
        <PlayerTarget ref={setContainerElement} />
        {videos.length > 1 && (
